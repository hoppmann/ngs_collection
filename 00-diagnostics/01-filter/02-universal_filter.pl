...
Options:
	...
	-db		name of the gemini database to use (mandatory option)

	-panel		name of the panel file to use (tab seperated list of gene and corresponding transcript)

	-outDir		name of the output directory (default = "09-filter")
	...
	-parentalIDs	the IDs of the parents to be used for trio filtering (requires exact 2 entries space seperated)
...
my $columns = "/data/programs/scripts/hoppmann/00-diagnostics/01-filter/01-columns.sh";
...
## check if trio setting is to use
if ( @parentalIDs){
	$trio = 1;
}
...
foreach my $pat (@patID){
	...
	my $panelFolder = "exome";
	if ($panel){
		$panelFolder = basename(dirname($panel));
	}

	my $outDir = "$outFolder/$pat/$panelFolder";
	...
	foreach my $curFilter (@difFilters){
		...
		## old version
		# my $mafAll = "aaf_gnomad_all";
		# my $mafAfr = "aaf_gnomad_afr";
		# my $mafEas = "aaf_gnomad_eas";
		# my $mafNfe = "aaf_gnomad_nfe";
		# my $mafSas = "aaf_gnomad_sas";

		my $mafAll = "alamut_gnomadAltFreq_all";
		my $mafAfr = "alamut_gnomadAltFreq_afr";
		my $mafEas = "alamut_gnomadAltFreq_eas";
		my $mafNfe = "alamut_gnomadAltFreq_nfe";
		my $mafSas = "alamut_gnomadAltFreq_sas";
		...
		###################
		######## MAF filter
		push(@sqlQuery, "($mafAll <= $maf or $mafAll is null) \\\n");
		push(@sqlQuery, "and ($mafAfr <= $maf or $mafAfr is null) \\\n");
		push(@sqlQuery, "and ($mafEas <= $maf or $mafEas is null) \\\n");
		push(@sqlQuery, "and ($mafNfe <= $maf or $mafNfe is null) \\\n");
		push(@sqlQuery, "and ($mafSas <= $maf or $mafSas is null) \\\n");



		##########################################
		######## exclude if no overlap with Alamut

		push(@sqlQuery, "and inAlamut == 1 \\\n");
