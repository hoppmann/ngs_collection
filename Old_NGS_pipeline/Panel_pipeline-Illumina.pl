...
my $picardDir = "/data/ngs/bin/picard/2.4.1";
my $picard = "$picardDir/picard.jar";
my $bedtoolsDir = "/data/ngs/bin/bedtools2-2.20.1/bin";
my $vcftools = "/data/ngs/bin/vcftools/0.1.12b/bin/vcftools";
...
my $hg19Dir = "/data/ngs/resources/bundle_2.8/ucsc.hg19.fasta";
my $indelMills = "/data/ngs/resources/bundle_2.8/Mills_and_1000G_gold_standard.indels.hg19.vcf";
my $indel1kgp = "/data/ngs/resources/bundle_2.8/1000G_phase1.indels.hg19.vcf";
my $snps1kgp = "/data/ngs/resources/bundle_2.8/1000G_phase1.snps.high_confidence.hg19.vcf";
my $dbsnp = "/data/ngs/resources/bundle_2.8/dbsnp_138.hg19.vcf";
my $exonFile = "/data/ngs/resources/exome_targets/SureSelect_V4_S03723314_Regions.bed";
my $hapmap = "/data/ngs/resources/bundle_2.8/hapmap_3.3.hg19.vcf";
my $omni = "/data/ngs/resources/bundle_2.8/1000G_omni2.5.hg19.vcf";

# misc
my $java1_8 = "/dsk/localall/lib/java/jre1.8.0_92_x64/bin/java";

#GATK
#my $gatk_dir = "/data/ngs/bin/GATK/3.3-0/";
my $gatk_dir = "/data/ngs/bin/GATK/3.6.0/";
...
#	my %bamFilesFromFile = ();
	...
	#get samtools version
	my $version = `$java1_8 -jar -$xmx $picardDir/picard.jar --help 2>&1 |grep Version`;
	chomp $version;
	...
	foreach my $currentPatient (@patients){
		...
		my $cmd = "$java1_8  -$xmx -Djava.io.tmpdir=$tmpDir \\\n";
		$cmd .= "-jar $picard MarkDuplicates \\\n";
		if ($bamIN) {
		...
		my $cmdRG = "$java1_8  -$xmx -Djava.io.tmpdir=$tmpDir \\\n";
		$cmdRG .= "-jar $picard AddOrReplaceReadGroups \\\n";
		$cmdRG .= "INPUT=$dirs{'duplicate'}/$currentPatient/$currentPatient.dup.bam \\\n";
		$cmdRG .= "OUTPUT=$dirs{'duplicate'}/$currentPatient.bam \\\n";
		$cmdRG .= "RGLB=$currentPatient RGPL=ILLUMINA RGPU=$currentPatient RGSM=$currentPatient \\\n";
		...
		####################################################
		######## Testing Purpose No Mark Duplicates ########
		####################################################
		
		$cmd = "cp $bamFilesFromFile{$currentPatient} $dirs{'duplicate'}/$currentPatient/$currentPatient.dup.bam";
...
if ($start > 5 or $end < 5) {
	...
	my $version = `$java1_8 -$xmx -jar $gatk --version`;
	chomp $version;
	...
	foreach my $currentPatient (@patients) {
		...
		my $cmdRealign = "$java1_8 -$xmx -jar $gatk \\\n";
...
if ($start > 6 or $end < 6) {
	...
	foreach my $currentPatient (@patients) {
		...
		my $cmdCovar = "$java1_8 -$xmx -jar $gatk -l INFO \\\n";
		$cmdCovar .= "-T BaseRecalibrator \\\n";
		$cmdCovar .= "-R $hg19Dir \\\n";
		...
		my $cmdPost = "$java1_8 -$xmx -jar $gatk -l INFO \\\n";
		$cmdPost .= "-T BaseRecalibrator \\\n";
		$cmdPost .= "-R $hg19Dir \\\n";
		...
		my $cmdPlot = "$java1_8 -$xmx -jar $gatk -l INFO \\\n";
		$cmdPlot .= "-T AnalyzeCovariates \\\n";
		$cmdPlot .= "-R $hg19Dir \\\n";
		...
		my $cmdPrint = "$java1_8 -$xmx -jar $gatk -l INFO \\\n";
		$cmdPrint .= "-T PrintReads  \\\n";
...
if ($start > 7 or $end < 7) {
	...
	foreach my $currentPatient (@patients) {
		...
		my $cmdAsMetrics = "$java1_8 -$xmx -Djava.io.tmpdir=$tmpDir \\\n";
		$cmdAsMetrics .= "-jar $picard CollectAlignmentSummaryMetrics \\\n";
		...
		my $cmdGc = "$java1_8 -$xmx -Djava.io.tmpdir=$tmpDir \\\n";
		$cmdGc .= "-jar $picard CollectGcBiasMetrics \\\n";
		...
		$cmdGc .= "SUMMARY_OUTPUT=$dirs{'stats'}/$currentPatient/$currentPatient.BCBMetrics.txt \\\n";
		$cmdGc .= "2> $dirs{'stats'}/$currentPatient/$currentPatient.GCBMetrics.log";


		#prepare command CollectInsertSiteMetrics (Picard)
		my $cmdIns = "$java1_8 -$xmx -Djava.io.tmpdir=$tmpDir \\\n";
		$cmdIns .= "-jar $picard CollectInsertSizeMetrics \\\n";	
		$cmdIns .= "VALIDATION_STRINGENCY=LENIENT \\\n";
		$cmdIns .= "INPUT=$dirs{'BQSR'}/$currentPatient.bam \\\n";
		$cmdIns .= "SUMMARY_OUTPUT=$dirs{'stats'}/$currentPatient/$currentPatient.ISMetrics.txt \\\n";
...
if ($start > 8 or $end < 8) {
	...
	foreach my $currentPatient (@patients){
		...
		my $cmdSnp = "$java1_8 -$xmx -jar $gatk -l INFO \\\n";
	...
	#prepare command 	
	my $cmdGenotypeGvcf = "$java1_8 -$xmx -jar $gatk -l INFO \\\n";
	...
	#prepare command
	my $cmdFilter = "$java1_8 -$xmx -jar $gatk -l INFO \\\n";
	...
	######## prepare commands
	#SNP
	my $cmdRecalSNP = "$java1_8 -$xmx -jar $gatk -l INFO \\\n";
	...
	#### apply recal
	my $cmdApplySnp = "$java1_8 -$xmx -jar $gatk -l INFO \\\n";
	...
	my $cmdRecalIndel = "$java1_8 -$xmx -jar $gatk -l INFO \\\n";
	...
	my $cmdApplyIndel = "$java1_8 -$xmx -jar $gatk -l INFO \\\n";
